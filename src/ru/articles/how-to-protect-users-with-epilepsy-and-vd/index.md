...
Недоступные интерфейсы могут спровоцировать любой из симпотомов. Факундо Коррадини в [статье на A List Apart](https://alistapart.com/article/accessibility-for-vestibular/) рассказывал как часами лежал в кровати с сильным головокружением после встреч с параллаксом на сайтах.
...
- aнимированная прокрутка, которая длится больше 1/4 секунды;
...
Частота обычных и красных вспышек, а также миганий должна быть не больше 3-х раз в секунду или 3 герца (Гц). Это минимальное требование по доступности для людей со светочувствительной эпилепсией.
...
{% note "Рекомендуемая область вспышек для дисплея с соотношением сторон 1024 на 768 и диагональю 15–17 дюймов при стандартном расстоянии до глаз (это 58–68 сантимеров) — 341 на 256 пикселей." %}
...
Вариант 1: только `prefers-reduced-motion`. Его [предлагает Вал Хед](https://github.com/jensimmons/cssremedy/issues/11#issuecomment-462867630).
...
Вариант 2: совмещаем `prefers-reduced-motion` и `update`.

[Медиафича `update`](https://www.w3.org/TR/mediaqueries-4/#update) определяет может ли устройство изменить внешний вид контента сайта, как только он отрендерился. Есть три значения: `none`, `slow` и `fast`.
...
Вариант 4, который подойдёт для браузерного расширения с вашими стилями:
...
- обновляется больше 5-ти секунд;
...
Когда параллакс-эффект ну очень важен, можно добавить альтернативные стили без анимации и опасных картинок или дайть ссылку на текстовую версию страницы уже без параллакса.
...
**Анимируйте текст с умом**. Анимированный текст тоже не самая безобидная часть интерфейса. Пока нет стандартных возможностей настроить его анимацию. Если он сильно смещается в сторону, ощутимо увеличиваетсяили уменьшается, то это тоже может вызвать приступ или головокружение. Так что лучше либо совсем отказаться от этой идеи, либо изменять контент незначительно и плавно.

**Не выжигайте пользователям глаза**. Высокая контрастность не всегда хорошо, особенно если это анимация. Хорошо, когда контрастность не слишком высокая и не слишком низкая или есть настройки цветовыз тем на самом сайте.

**Проверяйте картинки**. Следуйте паре простых рекомендаций о паттернах и картинках. Если графика состоит из прямых контрастных линий, то лучше остановиться на 8-ми. Если это волны, размещайте рядом не больше 5-ти.
