...
Exceptions to the minimum criterion include elements opened by users and components that can be moved around the page, by dragging, etc.

The enhanced criterion 2.4.12 doesn't allow even partially obscured elements in a focus state.

## Detailed explanation

Many of us have at least once found ourselves on a site with dropdown navigation where items are visually hidden but still accessible to the keyboard. Both criteria prevent this and similar navigation problems.

Elements with keyboard focus can also be obscured by:
...
**Fixed cookie banner** is another problematic design pattern. Banners with descriptive texts and controls are especially at risk of violating the level AA minimum criterion. In a lot of cases, cookie banners are placed on pages as the last element. Keyboard users need to go to the end of the page to close the cookie message.
...
A special case is **elements with glass effect or opaque styles** that overlay other content, such as semi-transparent tooltips and fixed site navigation.

To pass the level AA criterion 2.4.11 control elements must comply with another level AA criterion [1.4.11: Non-text Contrast](https://www.w3.org/TR/WCAG22/#non-text-contrast). To do this, check the contrast ratio between the color of the interactive element and the background of the overlaying element.

Imagine a semi-transparent tooltip that completely covers a link. You need to compare the link color resulting from blending with the tooltip background and the background of the pop-up hint itself.

For example, the link color is #232323 after blending with the background (young night,) and the background of the tooltip is #373737 (muted gray.) The contrast ratio between colors is 1.32:1. This is below the minimum value for non-text elements. Thus, the criterion 2.4.11 is failed.
...
In this imaginary example, we *did not compare* the color of the focus indicator. The only two cases we care about the appearance of the indicator are:

- When the focus indicator is placed inside the focused element
- When the element in the focus state changes significantly.

For example, a button in the focus state has an inner border, so we compare the border color with the background of the overlaying element. Another case is when a focused button changes the background color. Here we compare the new background color of the button with the background of the other element.

![Buttons are covered by a semi-transparent top layer.](images/focus-indicators-contrast.png)

**Elements that move around the page** will violate the minimum criterion when they obscure links, buttons, and interactive components by default before they are moved by users.
...
The next category of users are those who **use pointers for navigation** (excluded computer mice.) For instance, styluses, head pointers, and joysticks.

Don't forget about **users with cognitive differences** that affect memory and attention. For example, those with attention deficit and hyperactivity disorder (ADHD,) long COVID, or traumatic brain injuries (TBI.) These users often get distracted and forget about their current position and recent actions on the page.

## How to avoid barriers

The simplest solution is not to fix bulky navigation in the header and footer, especially on small screens. You can save screen space by reducing the sticky header height during scrolling.
...
- Determine the color of the focused element. Pick its original color without the focus indicator, mixed with the background of the overlaying element
- Check the contrast ratio between the colors.
...
If every interactive element in a focus state is visible completely or partially, the page complies with *2.4.11: Focus Not Obscured (Minimum) (Level AA)*.

If semi-transparent elements completely obscure elements in a focus state, and the contrast ratio between two colors is 3:1 and higher, the page also complies with the minimum level AA criterion.

When no element in the keyboard focus state is obscured by others, the page complies with *2.4.12: Focus Not Obscured (Enhanced) (Level AAA)*.
