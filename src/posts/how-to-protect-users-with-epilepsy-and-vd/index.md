...
Больше примеров проблемных интерфейсов есть в&nbsp;[тексте Эйлин Уэбб «Your Interactive Makes Me Sick»](https://source.opennews.org/articles/motion-sick/). Надеюсь, ваш вестибулярный аппарат выдержит это испытание лучше, чем&nbsp;мой.

<figure>

> Вам даже не нужны видео или&nbsp;изображение, чтобы&nbsp;причинить кому-то вред. Простой `<div>`, который с&nbsp;помощью JavaScript быстро меняет цвет и&nbsp;яркость, тоже может навредить. И мигание можно встретить где угодно. К примеру, спиннеры, которые показывают во время загрузки страниц, могут вращаться и мигать одновременно.
...
{% note "Рекомендуемая область вспышек для дисплея с&nbsp;соотношением 1024x768, диагональю 15–17&nbsp;дюймов при&nbsp;стандартном расстоянии до&nbsp;глаз (58–68&nbsp;см)&nbsp;— **341x256&nbsp;пикселей**." %}
...
В сниппете используется `slow`. Оно подходит для ситуаций, когда раскладка динамически изменяется в&nbsp;соответствии с&nbsp;обычными CSS-правилами, но&nbsp;устройство не отображает их плавно. Пример: экраны с&nbsp;электронными чернилами или&nbsp;дешёвые смартфоны.
...
👉 Не все пользователи продвинутые и&nbsp;знают, где находятся настройки анимации. Если&nbsp;учитываете человеческий фактор, то&nbsp;можно добавить специальную кнопку в меню. Она будет включать более безопасную анимацию. Так поступили на [сайте Animal Crossing](https://animal-crossing.com/).
...
👉 Анимированный текст тоже не самая безобидная часть интерфейса. Пока нет стандартных возможностей настроить его анимацию. Если он сильно смещается в&nbsp;сторону или&nbsp;ощутимо увеличивается/уменьшается, то&nbsp;это тоже может вызвать приступ или головокружение. Так что&nbsp;лучше либо&nbsp;совсем отказаться от&nbsp;этой идеи, либо&nbsp;изменять контент незначительно и&nbsp;плавно.

👉 Разместите предупреждение об&nbsp;пасном контенте, если&nbsp;сомневаетесь. Это спасёт ситуацию на какое-то время. Можно даже временно исключить такую страницу из выдачи. Например, с&nbsp;помощью HTML:

```HTML
<meta name="robots" content="noindex, nofollow">
```

Осталось только убедить маркетологов 😶

👉 Для паттернов или&nbsp;картинок тоже есть рекомендации. Если они состоят из прямых контрастных линий, то&nbsp;лучше остановиться на 8. Если они волнистые, то&nbsp;размещайте рядом не больше 5.
